            ...
            <div class="row pt-2">
                <div class="col ps-4">
                    <h1 class="display-6 mb-3">
                        <i class="bi bi-calendar2-week"></i> أخد الحضور
                    </h1>

                    @include('session-messages')

                    <h3><i class="bi bi-compass"></i>
                       الصف:{{request()->query('class_name')}}, 
                        @if ($academic_setting->attendance_type == 'course')
                           المادة: {{request()->query('course_name')}}
                        @else
                    ...
                    <div class="row mt-4">
                        <div class="col-10 bg-white border p-3 shadow-sm">
                            <form action="{{route('attendances.store')}}" method="POST">
                                ...
                                <div class="mb-4">
                                    <button type="submit" class="btn btn-outline-primary"><i class="bi bi-check2"></i> تقديم</button>
                                </div>
                                @endif
                            </form>
                        </div>
